...
Providers:
* ``railway`` — one Railway service per NEXO instance (per-instance public
  domain via Railway's API, WebSocket-capable, TLS at the edge). Selected
  automatically when NEXO_RAILWAY_TOKEN + project/environment are configured.
* ``local``   — NEXO Worker on a node running instances as containers
...
log = get("runtime", "nexo.console.deploy")
...
async def _pipeline_railway(pool: asyncpg.Pool, deployment_id: str, inst: dict,
    ...
    if not service_id:
        ...
        service_id = await provider.create_service(f"nexo-inst-{instance_id[:8]}")
        await pool.execute(
...
async def _pipeline_local(pool: asyncpg.Pool, deployment_id: str, inst: dict) -> None:
    ...
    await _set_deployment(pool, deployment_id, "building")
    await _log(pool, deployment_id, f"Preparing NEXO Core {inst['core_version']}…")
