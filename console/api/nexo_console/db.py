...
log = get("runtime", "nexo.console.db")
...
async def _connect_with_retry(dsn: str, ssl_ctx: ssl_module.SSLContext | None) -> asyncpg.Pool:
    ...
    for attempt in range(1, 31):
        try:
            ...
            raise RuntimeError(
                "PostgreSQL rejected the credentials in NEXO_DATABASE_URL / DATABASE_URL. "
...
async def init_pool() -> None:
    ...
    if not dsn:
        ...
        base = None
        for cand in (Path("/data"), Path.cwd() / ".nexo-data"):
            try:
                ...
                continue
        base = base or Path("/tmp/nexo-data")
        base.mkdir(parents=True, exist_ok=True)
        sqlite_dsn = f"sqlite:///{base / 'nexo.db'}"
