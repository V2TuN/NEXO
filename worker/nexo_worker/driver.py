"""Instance drivers for the NEXO Worker.

Two drivers share one interface:

* ``DockerDriver``   — production. Runs NEXO Core in an isolated container:
...
Selection: ``NEXO_WORKER_DRIVER=docker|process`` (default: docker if the
...
log = get("runtime", "nexo.worker.driver")
...
class DockerDriver(BaseDriver):
    ...
    def __init__(self, ports: PortAllocator, data_root: Path, network: str = "nexo"):
    ...
    async def launch(self, spec: LaunchSpec) -> InstanceHandle:
        ...
        name = f"nexo-inst-{spec.instance_id[:12]}"
        image = f"nexo/core:{spec.core_version}"
        ...
        try:
            await self._run(
                "run", "-d",
                "--name", name,
                "--label", "nexo.instance=" + spec.instance_id,
                "--label", "nexo.managed-by=nexo-worker",
                # resource limits
                ...
                "-e", f"NEXO_CORE_API_TOKEN={spec.api_token}",
                "-e", f"NEXO_PUBLIC_HOST={spec.public_host}",
                "-e", "NEXO_STATE_PATH=/data/state.json",
                "-v", f"{data_dir}:/data",
                image,
            )
        except DriverError:
...
class ProcessDriver(BaseDriver):
    ...
    def __init__(self, ports: PortAllocator, data_root: Path, core_cmd: list[str] | None = None):
        ...
        self.core_cmd = core_cmd or [
            os.environ.get("NEXO_CORE_PYTHON", ".venv/bin/python"),
            "-m", "nexo_core",
        ]
        self.core_cwd = os.environ.get("NEXO_CORE_CWD", "")
    ...
    async def launch(self, spec: LaunchSpec) -> InstanceHandle:
        ...
        env = os.environ.copy()
        env.update({
            "NEXO_CORE_API_TOKEN": spec.api_token,
            "NEXO_STATE_PATH": str(data_dir / "state.json"),
            "NEXO_PUBLIC_HOST": spec.public_host,
            "NEXO_LOG_JSON": "1",
            "PORT": str(port),
        })
...
def select_driver(data_root: Path) -> tuple[BaseDriver, str]:
    ports = PortAllocator(
        int(os.environ.get("NEXO_WORKER_PORT_START", "19000")),
        int(os.environ.get("NEXO_WORKER_PORT_END", "19999")),
    )
    chosen = os.environ.get("NEXO_WORKER_DRIVER", "").strip().lower()
    if not chosen:
