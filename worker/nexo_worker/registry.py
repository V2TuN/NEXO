"""Persistent instance registry for the NEXO Worker.
...
def _path() -> Path:
    base = Path(os.environ.get("NEXO_WORKER_DATA", "/data/instances"))
...
def save_instance(instance_id: str, data: dict) -> None:
    ...
    try:
        ...
        get("runtime", "nexo.worker.registry").warning("registry save failed: %s", exc)
